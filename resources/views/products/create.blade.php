...
<div class="max-w-2xl mx-auto">
    ...
    <form action="{{ route('products.store') }}" method="POST" class="glass p-8 rounded-3xl space-y-6">
        @csrf
        
        <div class="space-y-2">
            <label class="text-sm font-medium text-slate-400">Nom du produit</label>
            <input type="text" name="name" required placeholder="ex: MacBook Pro M3" 
        ...
        <div class="grid grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <input type="text" name="category" required placeholder="ex: Électronique" 
                    class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all">
            </div>
            <div class="space-y-2">
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" rows="3" placeholder="Détails du produit..." 
                class="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-600 transition-all"></textarea>
        </div>
